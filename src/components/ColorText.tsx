...
function parse(input: string): Chunk[] {
  ...
  while (i < input.length) {
    ...
    if (open === -1) {
      chunks.push({ text: input.slice(i) });
      break;
    }
    // push plain text before tag
    if (open > i) chunks.push({ text: input.slice(i, open) });

    const closeTag = input.indexOf("}", open + 1);
    if (closeTag === -1) {
      chunks.push({ text: input.slice(open) });
      break;
    }
    const tag = input.slice(open + 1, closeTag).trim(); // r / g / b / #RRGGBB
    const end = tag === "" ? -1 : input.indexOf("{/", closeTag + 1);
    if (end === -1) {
      // not a valid block, treat as plain
      chunks.push({ text: input.slice(open, closeTag + 1) });
      i = closeTag + 1;
      continue;
    }
    ...
    let color = NAMED[tag] || (tag.startsWith("#") ? tag : undefined);
